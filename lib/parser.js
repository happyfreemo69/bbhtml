...
Node.prototype.addText = function(txt){
    if(this.opts.escapeHtml){
        txt = this.opts.escapeHtml(txt)
    }
...
function Parser(options = {}){
    this.escapeHtml = options.escapeHtml===false?false:
        typeof(options.escapeHtml)=='function'?options.escapeHtml:
        function(txt){return txt.replace(/[<>"]/g,'')};
    this.nodeFactory = {
        get:tag=>{
            var N = this.nodes[tag.name];
            if(N){
                return new N(tag, {escapeHtml:this.escapeHtml});
            }
            return new Node(tag,{escapeHtml:this.escapeHtml});
        },
...
Parser.prototype.parse = function(str){
    ...
    var count = 0;
    this.currentNode = new RootNode(new Tag('_'),{escapeHtml:this.escapeHtml});//this tag must not be in this.nodes
